...
- [ ] M0.2 Add a backend registry/factory: `DriverName → constructor`, keyed off config. Replace `DriverIsSupported` static list with registry membership.
- [ ] M0.3 Make `Source` config driver-generic: add provider fields alongside Vault/AWS without breaking existing YAML; route `fetch.go` through the registry instead of hardcoded vault/aws branches.
...
- [ ] M2.2 Enhanced metrics: user-defined custom metrics via config; dependency-probe health endpoints.
