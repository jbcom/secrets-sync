...
vuln:
    ...
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" go run golang.org/x/vuln/cmd/govulncheck@v1.3.0 "${packages[@]}"
...
test-go *args:
    ...
    [[ -n "${packages_raw}" ]] || { echo "no Go packages discovered" >&2; exit 1; }
    mapfile -t packages <<<"${packages_raw}"
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" go test "${packages[@]}" {{ args }}
...
test-unit:
    ...
    [[ -n "${packages_raw}" ]] || { echo "no Go packages discovered" >&2; exit 1; }
    mapfile -t packages <<<"${packages_raw}"
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" go test -race -coverprofile=coverage.out "${packages[@]}"
...
build:
    ...
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" go build \
...
controller-build:
    ...
    commit="${COMMIT:-$(git rev-parse --short HEAD 2>/dev/null || echo none)}"
    date="${DATE:-$(date -u '+%Y-%m-%dT%H:%M:%SZ')}"
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" go build -trimpath \
      -ldflags "-s -w -X github.com/jbcom/secrets-sync/cmd/secrets-sync/cmd.Version=${version} -X github.com/jbcom/secrets-sync/cmd/secrets-sync/cmd.Commit=${commit} -X github.com/jbcom/secrets-sync/cmd/secrets-sync/cmd.Date=${date}" \
...
lambda-build:
    ...
    commit="${COMMIT:-$(git rev-parse --short HEAD 2>/dev/null || echo none)}"
    date="${DATE:-$(date -u '+%Y-%m-%dT%H:%M:%SZ')}"
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -trimpath \
      -ldflags "-s -w -X github.com/jbcom/secrets-sync/cmd/secrets-sync/cmd.Version=${version} -X github.com/jbcom/secrets-sync/cmd/secrets-sync/cmd.Commit=${commit} -X github.com/jbcom/secrets-sync/cmd/secrets-sync/cmd.Date=${date}" \
...
python-tools:
    mkdir -p .tools/bin
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" GOBIN="$PWD/.tools/bin" GOPY_VERSION="{{ gopy_version }}" X_TOOLS_VERSION="{{ x_tools_version }}" bash scripts/install-gopy.sh
...
fmt:
    ...
    [[ -n "${packages_raw}" ]] || { echo "no Go packages discovered" >&2; exit 1; }
    mapfile -t packages <<<"${packages_raw}"
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" go fmt "${packages[@]}"

# Run dependency cleanup.
tidy:
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" go mod tidy

# Download and tidy dependencies.
deps:
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" go mod download
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" go mod tidy
...
goreleaser-check:
    GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" go run github.com/goreleaser/goreleaser/v2@v2.16.0 check
...
test-integration:
    ...
    if [[ -z "${VAULT_ADDR:-}" || -z "${AWS_ENDPOINT_URL:-}" ]]; then
      ...
      GOTOOLCHAIN="${GO_TOOLCHAIN:-go1.26.6}" go test -v -tags=integration ./tests/integration/...
    fi
