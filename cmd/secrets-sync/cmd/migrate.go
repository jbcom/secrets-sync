...
var migrateCmd = &cobra.Command{
	...
	Long: `Migrate configuration from other secret management tools to SecretSync pipeline format.
...
# Generated by: secrets-sync migrate --from terraform-secretsmanager
#
...
`
	...
	fmt.Println("   3. Validate: secrets-sync validate --config " + outputFile)
	fmt.Println("   4. Dry run: secrets-sync pipeline --config " + outputFile + " --dry-run")
